...
rm ns.txt 
rm k8sresources.txt
rm -rf kube_resources

mkdir ./kube_resources
...
do
  ...
  do
	...
	:
        ...
        echo "--$ns--$resource---"
        echo "--------------------------------------------------"
        echo "--------------------------------------------------" >> ./kube_resources/$ns.txt
        echo "--$ns--$resource---" >> ./kube_resources/$ns.txt
        echo "--------------------------------------------------" >> ./kube_resources/$ns.txt
        kubectl -n $ns get $resource >> ./kube_resources/$ns.txt
    fi
  done
done < ns.txt
